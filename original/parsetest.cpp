...
void test_parsing() {
    auto results = parse_json(sample_reply);
    assert(results.size() == 5);
    assert(results[0].title == "Santa Barbara Gauchos vs Arizona Lax Cats | SLC Playoffs presented by Total Lacrosse");
    assert(results[0].description == "Subscribe to The Lacrosse Network for more great lacrosse content! http://www.youtube.com/subscription_center?add_user=TheLacrosseNetwork ...");
    assert(results[0].screenshot == "https://i.ytimg.com/vi/O-Q6lowLKXY/hqdefault.jpg");
}
